...
- ⚠️ **v0.5.62 起暂停**：实现方式需要逐条跳进详情页再滚动页面触发评论区加载，真机实证这一步会把整个 Obsidian 带崩（Windows 上表现为进程直接消失、日志戛然而止）。稳定性优先，先停掉；开关保留，等换成不滚动的方案再恢复。
- 暂停期间正文、转写、AI 总结全部正常，只是笔记里没有「精选评论」区块。
